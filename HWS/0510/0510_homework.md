...
- 단방향 데이터 흐름을 사용하면 하위 컴포넌트가 실수로 부모의 상태를 변경하여 앱의 데이터 흐름을 추론하기 더 어렵게 만드는 것을 방지할 수 있다.



#### 3. 아래와 같은 Vue 프로젝트에서 2개의 버튼이 동작하는 것을 비교하여 .native 수식어의 역할을 작성하시오.

- Vue.component에 Vue인스턴스의 이벤트를 바인딩 하고자 한다면, .native를 사용해 연결한다.
